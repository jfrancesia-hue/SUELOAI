...
export function Sidebar({ profile }: SidebarProps) {
  ...
  const cryptoEnabled = process.env.NEXT_PUBLIC_ENABLE_CRYPTO === 'true';
  const secondaryEnabled = process.env.NEXT_PUBLIC_ENABLE_SECONDARY_MARKET === 'true';
  ...
  const investorLinks = [
    ...
    ...(secondaryEnabled ? [{ href: '/secondary-market', label: 'Mercado Secundario', icon: RefreshCcw }] : []),
    { section: 'Cuenta' },
  ...
  const developerLinks = [
    ...
    { href: '/invoicing', label: 'FacturaciÃ³n', icon: Receipt },
    { section: 'Cuenta' },
    { href: '/verify', label: 'Verificar', icon: FileCheck },
    { href: '/notifications', label: 'Notificaciones', icon: Bell },
    { href: '/settings', label: 'Ajustes', icon: Settings },
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="px-4 py-4 border-b border-surface-200">
        <div className="flex items-center gap-3 px-2">
          <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-amber-500/20 to-brand-500/20 flex items-center justify-center shrink-0 border border-brand-500/10">
      ...
      <nav className="flex-1 px-3 py-3 space-y-0.5 overflow-y-auto">
        {links.map((item, i) => {
          if (item.section) {
            return (
              <p key={`section-${i}`} className="text-[10px] font-semibold text-surface-500 uppercase tracking-wider px-3 pt-4 pb-1.5">
                {item.section}
              </p>
            );
          }
            ...
            <Link
              ...
              className={cn(
                ...
                isActive
                  ...
                  : item.highlight
                    ? 'text-surface-900 bg-gradient-to-r from-brand-500/5 to-amber-500/5 border border-brand-500/10 hover:border-brand-500/20'
                    : 'text-surface-600 hover:text-surface-900 hover:bg-surface-200/50'
              )}
            >
              <Icon className={cn(
                'w-4 h-4 shrink-0',
                item.highlight && !isActive && 'text-brand-500'
              )} />
      ...
      <div className="px-3 py-3 border-t border-surface-200">
        <button
          ...
          className="flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium text-surface-500 hover:text-red-400 hover:bg-red-500/5 w-full transition-colors"
        >
    ...
    <>
      ...
      {mobileOpen && (
        <div className="lg:hidden fixed inset-0 z-50">
          ...
          <div className="absolute left-0 top-0 bottom-0 w-72 bg-surface-100 border-r border-surface-200 animate-slide-down">
      ...
      <div className="hidden lg:block fixed left-0 top-0 bottom-0 w-64 bg-surface-100 border-r border-surface-200 z-30">
        <SidebarContent />
      </div>
    </>
  );
